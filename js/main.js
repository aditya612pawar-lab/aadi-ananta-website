    ...
    navItems.forEach(item => {
        ...
        if (dropdown && link) {
            link.addEventListener('click', (e) => {
                ...
                // We prevent default only if it's strictly a dropdown toggle on smaller screens
                // or if the user specifically requested click-to-open logic.
                // Here, since the links are hashes (#wellness), preventing default doesn't stop much,
                // but we want to toggle the dropdown visibility.

                // If the link is just a toggle, we can prevent default.
                // However, user said "click on wellness living", which implies the link might also navigate.
                // Let's toggle the class.

                if (window.innerWidth < 992 || true) { // Apply to all for explicit "click" request
                    e.preventDefault();
    ...
    const serviceData = {
        ...
        'sports': {
            ...
            options: [
                ...
                { name: 'Wellness Sports', link: 'sports-wellness.html' }
            ]
        },
        'education': {
            title: 'Education',
