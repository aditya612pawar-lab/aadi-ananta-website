document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- NEW: Mobile Nested Submenu Handling ---
    ...
    dropdownItems.forEach(item => {
        ...
        if (submenu && link) {
            ...
            link.addEventListener('click', (e) => {
                // Only handle special logic on mobile
                if (window.innerWidth <= 900) {
                    e.preventDefault();
                    e.stopPropagation();

                    // Close other submenus at the same level (optional)
                    const parentMenu = item.closest('.dropdown-menu');
                    if (parentMenu) {
                        parentMenu.querySelectorAll('.dropdown-item').forEach(sibling => {
                            if (sibling !== item) {
    ...
    // Update click outside to also close submenus
    ...
    if (navContainer && navMenu) {
        ...
        navMenu.querySelectorAll('a').forEach(link => {
            link.addEventListener('click', (e) => {
                ...
                // BUT only if it doesn't have a nested submenu
                const isNestedParent = link.parentElement.classList.contains('has-submenu');

                if (!hasDropdown || (link.classList.contains('dropdown-link') && !isNestedParent)) {
